...
public class CustomizedCanalConnector implements CommandLineRunner {
    ...
    private void init() throws Exception {
        ...
        List<String> destinationList = Optional.of(canalClientConfig.getDestinations()).orElse(Collections.emptyList());
        if (destinationList.isEmpty()) {
            new Exception("destination is empty");
        }
        clientPool = Executors.newFixedThreadPool(destinationList.size());

        for (String destination : destinationList) {
            clientPool.submit(new CanalClient(destination,canalClientConfig));
        }
    }

    protected class CanalClient implements Callable<Void> {
        private boolean running = false;
        private String destination;
        private CanalConnector canalConnector;

        public CanalClient(String destination, CanalClientConfig canalClientConfig) {
            this.destination = destination;
            boolean isCluster = canalClientConfig.isCluster();
            if (isCluster) {
                canalConnector = CanalConnectors.newClusterConnector(canalClientConfig.getZkServers(), destination,
                ...
                canalConnector = CanalConnectors.newSingleConnector(
                        ...
                        destination, canalClientConfig.getUsername(), canalClientConfig.getPassword());
            }
        }
